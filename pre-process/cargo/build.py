...
from pipeline.layout import DEFAULT_INDEX_URL, CargoDataLayout
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        description="Manage a local crates.io-index clone and prepare a Cargo local-registry layout.",
    )
    common = argparse.ArgumentParser(add_help=False)
    common.add_argument(
        "--data-root",
        help="Managed Cargo preprocess data root. Defaults to pre-process/cargo/data or CARGO_PREPROCESS_DATA_ROOT.",
    )
    common.add_argument("--index-dir", help="Managed crates.io-index clone path.")
    common.add_argument("--local-registry-dir", help="Prepared local-registry output path.")
    ...
    subparsers.add_parser(
        "status",
        parents=[common],
        help="Show managed index and local-registry status.",
    )
...
def _build_status_payload(layout: CargoDataLayout, *, operation: str) -> dict[str, object]:
    return {
        "status": "ok",
        "operation": operation,
...
def main(argv: list[str] | None = None) -> int:
    ...
    layout = CargoDataLayout.from_overrides(
        ...
        local_registry_dir=args.local_registry_dir,
    )

    try:
        if args.command == "clone":
            clone_index(args.index_url, layout.index_dir, force=args.force)
            payload = _build_status_payload(layout, operation="clone")
            payload["source"] = {"index_url": args.index_url}
        elif args.command == "update":
            update_result = update_index(layout.index_dir)
            payload = _build_status_payload(layout, operation="update")
            payload["update"] = update_result
        elif args.command == "status":
            payload = _build_status_payload(layout, operation="status")
            ...
            payload = _build_status_payload(layout, operation="prepare-local-registry")
