...
- Files: `resolving/containerization/docker-compose.yml`, `resolving/containerization/runtime/default_adapter.py`, `resolving/containerization/images/pip/Dockerfile`
...
- Issue: `resolving/config/resolvers.container.yaml` and `resolving/config/resolvers.yaml` are parsed with `json.load(...)`.
...
- Files: `resolving/containerization/images/cargo/src/batch/config.rs`, `resolving/containerization/images/cargo/src/batch/db.rs`
...
- Files: `resolving/containerization/images/npm/src/main.cpp`
...
- Trigger: Broad scans over `resolving/containerization/images/npm/third_party/`.
- Files: `resolving/containerization/images/npm/third_party/`
...
- Files: `resolving/containerization/images/cargo/src/batch/config.rs`
...
**resolving execution is config-driven subprocess execution:**
- Risk: The gateway runs the command arrays declared in resolver config files through `subprocess.run(...)`.
- Files: `resolving/gateway/runner.py`, `resolving/gateway/config.py`, `resolving/config/resolvers.container.yaml`
...
- Files: `resolving/containerization/docker_gateway_proxy.py`
...
- Files: `resolving/containerization/images/npm/src/dataset.cpp`, `resolving/containerization/images/npm/src/dataset.hpp`
...
- Safe modification: Change one adapter/backend pair at a time and validate against `resolving/spec/request.schema.json` and `resolving/spec/response.schema.json`.
...
- Files: `main.py`, `resolving/containerization/docker_gateway_proxy.py`, `resolving/containerization/runtime/*.py`
...
- Files: `resolving/containerization/images/npm/third_party/`, `resolving/containerization/images/npm/CMakeLists.txt`
...
- Files: `resolving/containerization/images/maven/pom.xml`
- Impact: Upgrading Java or repository behavior may be harder than with a newer dependency resolver stack.
- Migration plan: Evaluate a modern Maven resolving upgrade path before expanding Maven-specific functionality.
