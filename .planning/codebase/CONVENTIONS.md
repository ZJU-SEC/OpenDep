...
**Files:**
- Python modules use `snake_case.py`, for example `resolving/gateway/response.py` and `resolving/containerization/runtime/go_adapter.py`.
- Go packages use short lowercase directory names such as `internal/source`, `internal/output`, and `internal/parser` under `resolving/containerization/images/go/`.
- Rust modules and binaries use `snake_case.rs`, for example `resolving/containerization/images/cargo/src/bin/cargo_resolver.rs`.
- Java files use `PascalCase.java`, matching class names in `resolving/containerization/images/maven/src/main/java/`.
- C++ sources use lowercase names with `.cpp` / `.hpp`, for example `dataset.cpp`, `idealTree.cpp`, and `config.hpp` in `resolving/containerization/images/npm/src/`.
...
- Python code uses 4-space indentation, type hints, and `from __future__ import annotations` in nearly every module under `resolving/gateway/` and `resolving/containerization/runtime/`.
- Go code appears to follow standard `gofmt` layout and import grouping, as seen in `resolving/containerization/images/go/cmd/go_resolver/main.go`.
- Rust code appears to follow idiomatic `rustfmt` style with grouped `use` statements and compact CLI parsing in `resolving/containerization/images/cargo/src/bin/cargo_resolver.rs`.
- C++ code uses 2-space indentation and preprocessor-heavy configuration in `resolving/containerization/images/npm/src/`.
...
2. Local package imports second, usually from `resolving.gateway` or `resolving.containerization.runtime`.
...
**Grouping:**
- Python files often insert a blank line between bootstrap path logic and local imports, as in `main.py` and `resolving/containerization/docker_gateway_proxy.py`.
...
**Path Conventions:**
- Python import paths are absolute within the repo package namespace, for example `from resolving.gateway.response import GatewayResponseFactory`.
- Go uses package-local internal imports rooted at `github.com/package-dependency/go-resolver/...`.

## Error Handling

**Patterns:**
- Python host code uses custom exception classes derived from `GatewayError` in `resolving/gateway/errors.py`, then catches them at the service boundary in `resolving/gateway/service.py`.
- Gateway request and response validation return lists of string errors rather than raising directly in `resolving/gateway/contract.py`.
- Runtime adapters convert backend failures into structured `error_response(...)` payloads in `resolving/containerization/runtime/adapter_runtime.py`.
...
- Rust batch utilities use `simplelog` in `resolving/containerization/images/cargo/src/batch/config.rs`.
- Java backend logging uses SLF4J plus Logback from `resolving/containerization/images/maven/pom.xml` and `resolving/containerization/images/maven/src/main/resources/logback.xml`.
- The npm backend uses `fmt::print` and stderr logging in `resolving/containerization/images/npm/src/main.cpp` and `dataset.cpp`.
...
- The C++ backend contains inline TODO and XXX markers in files such as `resolving/containerization/images/npm/src/dataset.cpp` and `resolving/containerization/images/npm/src/node.hpp`.
...
**Size:**
- Python gateway functions are generally small and single-purpose, especially in `resolving/gateway/service.py`, `registry.py`, `router.py`, and `models.py`.
...
- Rust exposes shared APIs from `resolving/containerization/images/cargo/src/lib.rs` and keeps CLI entrypoints in `src/bin/`.

**Boundaries:**
- The Python gateway is intentionally thin and ecosystem-agnostic.
- Ecosystem-specific semantics live inside `resolving/containerization/runtime/<ecosystem>_adapter.py` and the native backend directories under `resolving/containerization/images/`.
- Documentation files under `README.md` and `resolving/**/README.md` are part of the repo’s operating conventions and are worth updating alongside behavior changes.
