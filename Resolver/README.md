...
The `Resolver/` tree is responsible for:
...
In the current structure, users interact with the repository-root `main.py`, while the code in `Resolver/` provides the supporting implementation.
...
main.py
  ...
  -> Resolver/containerization/images/<ecosystem>/
  -> normalized response
```
...
- `Resolver/` is not a separate end-user CLI directory
- `Resolver/gateway/` is an internal implementation layer
- `Resolver/config/` and `Resolver/spec/` support the runtime and contract model
- `Resolver/containerization/` contains the current backend integration strategy

## Recommended reading order

If you are new to this subtree, a good reading order is:

1. `Resolver/spec/README.md`
2. `Resolver/config/README.md`
3. `Resolver/gateway/README.md`
4. `Resolver/containerization/README.md`
...
- The gateway-level request and response validation logic currently lives in `Resolver/gateway/contract.py`.
