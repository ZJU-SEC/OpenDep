# Resolver Runtime Adapters

This directory contains the container-side runtime adapters used by the OpenDep resolver stack.
These adapters are the protocol bridge between the gateway request model and the native backend binaries, jars, or placeholder runtimes packaged in `Resolver/containerization/images/`.

## Purpose

The files in `Resolver/containerization/runtime/` are responsible for the container-facing execution layer.
...
They are typically invoked by `docker compose` services defined in `Resolver/containerization/docker-compose.yml`.

## Relationship to adjacent directories

Within the containerized resolver stack:

- `Resolver/containerization/images/` builds the native backend images
- `Resolver/containerization/runtime/` provides the adapter entrypoints that run inside those images
- `Resolver/containerization/docker-compose.yml` selects one adapter per service as the container entrypoint
- `Resolver/config/` stores resolver registry files used by the gateway
- `Resolver/spec/` documents the shared wire protocol used by the gateway and adapters
...
- `Resolver/containerization/runtime/pip_adapter.py` — adapter for the Python pip backend
- `Resolver/containerization/runtime/npm_adapter.py` — adapter for the native npm C++ backend
- `Resolver/containerization/runtime/maven_adapter.py` — adapter for the native Maven Java backend
- `Resolver/containerization/runtime/cargo_adapter.py` — adapter for the native Cargo Rust backend
- `Resolver/containerization/runtime/go_adapter.py` — adapter for the native Go backend
- `Resolver/containerization/runtime/default_adapter.py` — generic placeholder adapter kept for future incomplete integrations
...
- `Resolver/containerization/runtime/adapter_runtime.py`
- `Resolver/containerization/runtime/launcher_normalization.py`
...
- Adapter configuration is driven primarily by environment variables injected by `Resolver/containerization/docker-compose.yml`.
