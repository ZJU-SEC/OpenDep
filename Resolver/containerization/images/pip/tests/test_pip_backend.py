...
from Resolver.containerization.images.pip.backend import cli
        ...
        with tempfile.TemporaryDirectory(prefix="pip-live-test-") as temp_dir:
            ...
            completed = subprocess.run(
                [
                    sys.executable,
                    "-m",
                    "Resolver.containerization.images.pip.backend",
                    "resolve",
                    "--name",
                    "rootpkg",
                    "--format",
...
class PipAdapterTests(unittest.TestCase):
    def _run_adapter(self, module_name: str, module_source: str, request: dict[str, object]) -> tuple[int, dict[str, object]]:
        with tempfile.TemporaryDirectory(prefix="pip-adapter-test-") as temp_dir:
            ...
            completed = subprocess.run(
                [sys.executable, "Resolver/containerization/runtime/pip_adapter.py"],
                cwd=PROJECT_ROOT,
                env=env,
...
class ContainerConfigTests(unittest.TestCase):
    def test_compose_and_registry_reference_real_pip_backend(self) -> None:
        compose_text = (PROJECT_ROOT / "Resolver/containerization/docker-compose.yml").read_text(encoding="utf-8")
        self.assertIn('entrypoint: ["python3", "Resolver/containerization/runtime/pip_adapter.py"]', compose_text)
        self.assertNotIn('command: ["python", "Resolver/containerization/runtime/default_adapter.py"]', compose_text)
        self.assertIn("pip_cache", compose_text)

        registry = json.loads(
            (PROJECT_ROOT / "Resolver/config/resolvers.container.yaml").read_text(encoding="utf-8")
        )
...
class BackendDescribeTests(unittest.TestCase):
    def test_backend_describe_matches_completed_task_state(self) -> None:
        completed = subprocess.run(
            [sys.executable, "-m", "Resolver.containerization.images.pip.backend", "describe"],
            cwd=PROJECT_ROOT,
            capture_output=True,
            text=True,
            check=False,
