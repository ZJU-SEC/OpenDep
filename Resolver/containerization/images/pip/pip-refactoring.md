...
当前仓库中，新的容器化 pip backend 已经落地在 `Resolver/containerization/images/pip/backend/`，老逻辑仍保留在 `Resolver/containerization/images/pip/.legacy/ModuleGuard/` 作为参考实现。
...
建议在 `Resolver/containerization/images/pip/` 下新增真正的 backend 代码，例如：
...
并在 `Resolver/containerization/runtime/` 下新增：
...
- 更新 `Resolver/containerization/images/pip/Dockerfile`
