...
本文档用于在重构 Python 后端 Resolver 之前，先把当前 `Resolver/` 子系统的实际架构、四个已接入生态（`maven`、`npm`、`cargo`、`go`）的实现方式，以及 `pip` 目前所处的位置理清楚。
...
main.py
  -> Resolver/gateway/
  -> Resolver/config/resolvers.container.yaml
  ...
  -> Resolver/containerization/images/<ecosystem>/ 中的原生后端
  -> adapter 输出统一 JSON response
```

这条链路里最关键的设计点有两个：

1. `main.py` 和 `Resolver/gateway/` 不直接理解各生态的依赖解析逻辑，它们只负责统一协议、路由、执行和错误包装。
...
- 调用 `Resolver.gateway.service.GatewayService`；
- 打印统一 response JSON。
...
### 2.2 Gateway 层：`Resolver/gateway/`
...
### 2.3 Registry 层：`Resolver/config/`

当前有两份 registry：

- `Resolver/config/resolvers.container.yaml`
- `Resolver/config/resolvers.yaml`

实际生效的是前者。

原因有两个：

1. `Resolver/gateway/config.py` 的 `default_config_path()` 会优先选择 `resolvers.container.yaml`。
2. `resolvers.yaml` 里指向的老路径，如：
   - `Resolver/pip-dependency-resolver/adapter_main.py`
   - `Resolver/maven-dependency-resolver/adapter_launcher.py`
   - `Resolver/go-dependency-resolver/adapter_launcher.py`
   - `Resolver/cargo-dependency-resolver/adapter_launcher.py`
   - `Resolver/js-dependency-resolver/adapter_launcher.py`
...
### 2.4 统一协议：`Resolver/spec/`

这里定义的是跨生态共享的 request/response contract。

重点包括：

- 请求 schema：`Resolver/spec/request.schema.json`
- 响应 schema：`Resolver/spec/response.schema.json`
- 错误码 taxonomy：
  - `INVALID_ARGUMENT`
  - `UNSUPPORTED_COMMAND`
...
`Resolver/containerization/` 里其实又分成三层。
...
### 3.3 容器内 adapter 层：`Resolver/containerization/runtime/`
...
目录：`Resolver/containerization/images/maven/`
...
后端使用 Eclipse Aether / Maven Resolver 相关能力：
...
目录：`Resolver/containerization/images/npm/`
...
目录：`Resolver/containerization/images/cargo/`
...
目录：`Resolver/containerization/images/go/`
...
重构前，`Resolver/containerization/images/pip/` 只有一个 `Dockerfile`：
...
CMD ["python", "Resolver/containerization/runtime/default_adapter.py"]
```
...
- image 使用 `Resolver/containerization/images/pip/Dockerfile`
- entry command 使用 `Resolver/containerization/runtime/default_adapter.py`
...
- `Resolver/containerization/images/pip/`
  ...
  - `Dockerfile`
  - 可选 `README.md`
- `Resolver/containerization/runtime/pip_adapter.py`
...
2. 再把 Python pip resolver 后端放进 `Resolver/containerization/images/pip/`。
3. 让后端先稳定输出 JSON graph。
4. 最后更新：
   - `Resolver/containerization/images/pip/Dockerfile`
   - `Resolver/containerization/docker-compose.yml`
...
5. 新的 Python pip resolver 最自然的接入点是：
   - 在 `Resolver/containerization/images/pip/` 中放真实后端；
   - 在 `Resolver/containerization/runtime/pip_adapter.py` 中对齐现有 adapter 模式。
