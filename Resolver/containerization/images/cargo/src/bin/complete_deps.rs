...
use rust_deps::batch::config::{
    init_file_logger, BatchTables, VersionInfo, COMPLETE_DEPS_LOG_FILE, COMPLETE_DEPTH_TABLES,
    THREAD_DATA_SIZE,
};
use rust_deps::batch::db::{
    connect_default, fetch_versions_by_status, get_ver_name_table, insert_complete_dependencies,
...
fn main() {
    init_file_logger(COMPLETE_DEPS_LOG_FILE).unwrap();
...
pub fn count_all_deps(workers: usize, status: &str) {
    ...
    let tables = COMPLETE_DEPTH_TABLES;
            ...
            while let Ok(versions) = rx.recv() {
                for version in versions {
                    ...
                    if catch_unwind({
                        ...
                        move || {
                            if let Err(error) = resolve(Arc::clone(&conn), &version, Arc::clone(&ver_name_table), tables) {
                                warn!(
...
fn resolve(
    ...
    let result = resolve_store_fulldeps_of_version(Arc::clone(&conn), version_info, ver_name_table, tables);
...
fn resolve_store_fulldeps_of_version(
    ...
    insert_complete_dependencies(conn, tables, version_info.version_id, &set);
    Ok(())
}
